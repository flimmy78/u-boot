...
#ifdef SUPPORT_SWITCH_PHY
#define PHY_ID_NONE 	0xFFFFFFFF
#define PHY_ID_IP175D 0x02430d80 //It's same with IP175D
#define PHY_ID_IP1725 0x02431725 //Please notes it's dummy
#define PHY_TYPE_NAME "phytype"
#define PHY_TYPE_IP175D_NAME "ip175d"
#define PHY_TYPE_IP1725_NAME "ip1725"
static unsigned long swt_get_phy_id(void)
{
	char * phytype;
	static int phy_first_use = 1;

	phytype = getenv(PHY_TYPE_NAME);
	if (phytype) {
		if(phy_first_use)
		{
			printf("phytype is equal %s\n", phytype);
			phy_first_use = 0;
		}
		if(strcmp(phytype,PHY_TYPE_IP175D_NAME) == 0)
			return PHY_ID_IP175D;
		else if(strcmp(phytype,PHY_TYPE_IP1725_NAME) == 0)
			return PHY_ID_IP1725;
	}

	if(phy_first_use)
	{
		printf("if use switch,please set phytype to special type\n");
		phy_first_use = 0;
	}
...
static int swt_miiphy_read (char *devname, unsigned char addr, unsigned char reg,
		 unsigned short *value)
{
	...
	{
		case PHY_BMCR:
			*value = PHY_BMCR_DPLX | PHY_BMCR_100MB;  //Duplex and 100Mbps
			break;
		case PHY_BMSR:
			*value = PHY_BMSR_LS;  //Link
			break;
		case PHY_PHYIDR1:
			*value = swt_get_phy_id() >> 16;
			break;
		case PHY_PHYIDR2:
			*value = swt_get_phy_id() & 0xFFFF;
			break;
		default:
			*value = 0xFFFF;
			ret = 1;
		 ...
		 unsigned short *value)
{
	...
	list_for_each (entry, &mii_devs) {
		...
		if (strcmp (devname, dev->name) == 0) {
			found_dev = 1;
			#ifndef SUPPORT_SWITCH_PHY
				read_ret = dev->read (devname, addr, reg, value);
			#else
				if(swt_get_phy_id() != PHY_ID_NONE )
					read_ret = swt_miiphy_read(devname, addr, reg, value);
				else
					read_ret = dev->read (devname, addr, reg, value);
			#endif
